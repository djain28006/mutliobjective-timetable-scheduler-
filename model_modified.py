...
def build_model():
    ...
    # ---- CMPM Honours constraint (block from mid-day and after OE) ----------
    CMPM_THEORY_IDX = 4
    for div in range(NUM_DIVS):
        for day in range(NUM_DAYS):
            # Block globally from H3, H4, H6, H7 (indices 2, 3, 5, 6)
            for s in [2, 3, 5, 6]:
            ...
            # Block from H3 on OE-2hr days to prevent gap for non-CMPM students
            model.Add(tv[(div, CMPM_THEORY_IDX, day, 2)] == 0).OnlyEnforceIf(oe2[day])
        ...
        for day in range(NUM_DAYS):
            ...
            for s in ACADEMIC_SLOTS:
                ...
                if terms:
                    ...
                    model.Add(act_s == 0)
                active_physical[s] = act_s

            is_active_any = model.NewBoolVar(f'stu_act_any_{div}_{day}')
            model.Add(div_day_hours[(div, day)] > 0).OnlyEnforceIf(is_active_any)
            model.Add(div_day_hours[(div, day)] == 0).OnlyEnforceIf(is_active_any.Not())

            first_s = model.NewIntVar(0, 9, f'stu_first_{div}_{day}')
            first_exprs = []
            for s in ACADEMIC_SLOTS:
                v = model.NewIntVar(0, 9, f'f_expr_{div}_{day}_{s}')
                model.Add(v == s).OnlyEnforceIf(active_physical[s])
                model.Add(v == 9).OnlyEnforceIf(active_physical[s].Not())
                first_exprs.append(v)
            model.AddMinEquality(first_s, first_exprs)

            last_s = model.NewIntVar(0, 9, f'stu_last_{div}_{day}')
            last_exprs = []
            for s in ACADEMIC_SLOTS:
                v = model.NewIntVar(0, 9, f'l_expr_{div}_{day}_{s}')
                model.Add(v == s).OnlyEnforceIf(active_physical[s])
                model.Add(v == 0).OnlyEnforceIf(active_physical[s].Not())
                last_exprs.append(v)
            model.AddMaxEquality(last_s, last_exprs)

            # ST5: Campus stay
            campus_stay = model.NewIntVar(0, 9, f'stu_cstay_{div}_{day}')
            model.Add(campus_stay == last_s - first_s + 1).OnlyEnforceIf(is_active_any)
            model.Add(campus_stay == 0).OnlyEnforceIf(is_active_any.Not())
            student_penalties.append(W_STU_CAMPUS_STAY * campus_stay)

            # ST1: HARD CONSTRAINT: Student No-Gap Pre-Break and Post-Break
            # Pre-break block: H1-H4 (slots 0-3)
            # If a slot is empty, either all slots before it are empty OR all slots after it are empty.
            for s in range(0, 4):
                ...
                if s == 0:
                    ...
                    model.Add(sum(active_physical[i] for i in range(0, s)) == 0).OnlyEnforceIf(before_empty)
                    model.Add(sum(active_physical[i] for i in range(0, s)) > 0).OnlyEnforceIf(before_empty.Not())
                    
                if s == 3:
                    model.Add(after_empty == 1)
                else:
                    model.Add(sum(active_physical[i] for i in range(s+1, 4)) == 0).OnlyEnforceIf(after_empty)
                    model.Add(sum(active_physical[i] for i in range(s+1, 4)) > 0).OnlyEnforceIf(after_empty.Not())
                    
                # If slot is inactive, either before is empty or after is empty
                model.AddBoolOr([active_physical[s], before_empty, after_empty])
                
            # Post-break block: H6-H9 (slots 5-8)
            for s in range(5, 9):
                before_empty = model.NewBoolVar(f'stu_be_{div}_{day}_{s}')
                after_empty = model.NewBoolVar(f'stu_ae_{div}_{day}_{s}')
                ...
                if s == 5:
                    ...
                    model.Add(sum(active_physical[i] for i in range(5, s)) == 0).OnlyEnforceIf(before_empty)
                    model.Add(sum(active_physical[i] for i in range(5, s)) > 0).OnlyEnforceIf(before_empty.Not())
                    
                if s == 8:
                    model.Add(after_empty == 1)
                else:
                    model.Add(sum(active_physical[i] for i in range(s+1, 9)) == 0).OnlyEnforceIf(after_empty)
                    model.Add(sum(active_physical[i] for i in range(s+1, 9)) > 0).OnlyEnforceIf(after_empty.Not())
